      ...
      <div className="mx-auto -mt-16 max-w-6xl px-5 sm:px-8">
        ...
        <section className="mt-9">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {mainMenus.map((item) => { const Icon = item.icon; const linkProps = item.slug === "buku" ? { to: "/buku" as const } : item.slug === "huruf-hangeul" ? { to: "/hangeul" as const } : item.slug === "kosa-kata" ? { to: "/kosa-kata" as const } : item.slug === "gambar" ? { to: "/gambar" as const } : item.slug === "tata-bahasa" ? { to: "/tata-bahasa" as const } : item.slug === "uji-kemampuan" ? { to: "/uji-kemampuan" as const } : item.slug === "budaya-informasi" ? { to: "/budaya-informasi" as const } : { to: "/fitur/$slug" as const, params: { slug: item.slug } }; return (
