      ...
      <div className="mx-auto -mt-16 max-w-6xl px-5 sm:px-8">
        ...
        <section className="mt-8">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {quickMenus.map((item) => { const Icon = item.icon; return (
              <Link key={item.label} {...(item.slug === "buku" ? { to: "/buku" as const } : { to: "/fitur/$slug" as const, params: { slug: item.slug } })} className="group flex min-w-0 items-center gap-3 rounded-xl border border-border bg-surface p-4 shadow-soft transition-transform hover:-translate-y-0.5">
        ...
        <section className="mt-9">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {mainMenus.map((item) => { const Icon = item.icon; const linkProps = item.slug === "buku" ? { to: "/buku" as const } : { to: "/fitur/$slug" as const, params: { slug: item.slug } }; return (
